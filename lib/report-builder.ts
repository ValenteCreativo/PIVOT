...
function sponsorFit(assessment:MentorModelAssessment,research:ResearchRound[]){const evidence=research.flatMap(r=>r.findings).filter(f=>!f.demo).map(f=>`${f.title} ${f.summary} ${f.claimSupported}`).join(' ').toLowerCase();const verified=assessment.sponsorFit.filter(s=>s.fit==='UNKNOWN'||(s.name.length>=3&&evidence.includes(s.name.toLowerCase())));const named=verified.filter(s=>s.fit!=='UNKNOWN');return named.length?named:[{name:'Hackathon sponsors',fit:'UNKNOWN' as const,reason:'Sponsor and track information could not be verified from the available target-hackathon evidence.'}];}
